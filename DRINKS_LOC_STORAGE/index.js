...
let foodStorage = new HashStorage();

let drinkLocalStorage = new LocStorage("drinks");
let foodLocalStorage = new LocStorage('food');

if(localStorage.length > 0) {
  if(localStorage.getItem("drinks")) {
    drinkStorage.storage = JSON.parse(localStorage.getItem("drinks"));
    drinkLocalStorage.locHash = JSON.parse(localStorage.getItem("drinks"));
  }

  if(localStorage.getItem("food")) {
    foodStorage.storage = JSON.parse(localStorage.getItem("food"));
    foodLocalStorage.locHash = JSON.parse(localStorage.getItem("food"));
  }
}
...
addButton.addEventListener('click', function () {
  ...
  drinkLocalStorage.addToLocal(drinkName, dataValue);
...
deleteValueButton.addEventListener('click', function () {
  ...
  drinkLocalStorage.deleteFromLocal(key);
...
addButtonFood.addEventListener('click', function () {
  ...
  foodLocalStorage.addToLocal(foodName, dataValue);
...
deleteValueButtonFood.addEventListener('click', function () {
  ...
  foodLocalStorage.deleteFromLocal(key);
